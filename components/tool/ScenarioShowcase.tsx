    ...
    <div className={`grid gap-3 ${getGridLayout(allImages.length)}`}>
      {allImages.map((image, index) => (
        ...
        >
          ...
          >
            <p className="text-white text-xs text-center font-medium leading-relaxed">{image.prompt}</p>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="w-full max-w-7xl mx-auto p-6">
      ...
      <div className="space-y-24">
        {scenarioShowcase.scenarios.map((scenario, index) => {
          ...
          const allImages = [...scenario.inputImage, ...scenario.outputImages]
          ...
          return (
            <div key={scenario.id} className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-stretch">
              ...
              <div className={`h-full ${isEven ? "lg:order-2" : "lg:order-1"}`}>
                ...
                {allImages.length === 0 ? <EmptyImageSection title={scenario.category} index={index} /> : <ImageSection images={allImages} />}
              </div>
            </div>
          )
        })}
