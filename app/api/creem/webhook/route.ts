...
import { RECHARGES, SubscriptionTrialCredits } from '@/lib/ai-tools/products';
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (webhook.eventType === "subscription.paid") {
            ...
            const product = RECHARGES.find(p => p.productId === productId);
            ...
            await prisma.$transaction(async (tx) => {
                ...
                await tx.creditTransaction.create({
                    data: {
                        ...
                        description: `get ${product.credit} credits for subscription to ${product.name}`,
